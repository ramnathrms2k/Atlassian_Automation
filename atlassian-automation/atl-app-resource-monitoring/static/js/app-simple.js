(function () {
  ...
  function fetchMetrics() {
    ...
    return fetch(apiUrl('/api/metrics'))
      ...
      .then(function (data) {
        ...
        if (data.error) {
          if (serverCards) serverCards.innerHTML = '<div class="card"><div class="card-header"><span class="card-error">' + escapeHtml(data.error) + '</span></div></div>';
          if (dbCards) dbCards.innerHTML = '';
        } else {
          const list = data.servers || [];
          if (serverCards) serverCards.innerHTML = list.map(renderServer).join('');
          const dbList = data.db_nodes || [];
          if (dbCards) dbCards.innerHTML = dbList.length ? dbList.map(renderDbNode).join('') : '<p class="section-empty">No DB nodes discovered from app config.</p>';
        }
        setLastUpdated();
        setStatus('');
        return data;
      ...
      .catch(function (e) {
        if (loading) loading.style.display = 'none';
        if (serverCards) serverCards.innerHTML = '<div class="card"><div class="card-header"><span class="card-error">Request failed: ' + escapeHtml(e.message) + '</span></div></div>';
  ...
  function startMonitoring() {
    ...
    const sec = intervalInput ? Math.max(10, parseInt(intervalInput.value, 10) || 60) : 60;
  ...
  function startAutoRefresh() {
    stopAutoRefresh();
    var sec = Math.max(10, defaultInterval || 60);
  ...
  function loadConfigAndMetrics() {
    ...
    fetch(apiUrl('/api/config'))
      ...
      .catch(function (e) {
        setStatus('Config failed: ' + (e && e.message ? e.message : 'check server logs'), true);
        if (loading) loading.style.display = 'none';
      });
  }
