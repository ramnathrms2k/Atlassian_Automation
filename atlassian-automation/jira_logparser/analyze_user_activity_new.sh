...
# 
# Usage: ./analyze_user_activity.sh <USERID> <DATE> [BUNDLE_TYPE] [URI_PATTERN]
#        ./analyze_user_activity.sh jd008420 2025-09-12 jira
#        ./analyze_user_activity.sh mo024066 2025-09-14 confluence
#        ./analyze_user_activity.sh user123 2025-09-15 jira '/rest/api/2/issue'
# 
# Author: SRE Team
# Version: 1.0
# ========================================================================================
...
if [ $# -lt 2 ]; then
    ...
    echo "  URI_PATTERN - Optional: regex pattern to filter specific URIs"
    echo ""
    echo "Examples:"
    ...
    echo ""
    echo "URI_PATTERN examples:"
    echo "  '/rest/api/2/issue'     - Jira issue API calls"
    echo "  '/rest/api/content'     - Confluence content API calls"
    echo "  '/secure/Dashboard'     - Jira dashboard pages"
    echo "  '/pages/viewpage'       - Confluence page views"
    exit 1
fi
...
analyze_confluence_logs() {
    ...
    echo "$extracted_data" | awk -F'|' -v userid="$userid" -v target_date="$date_dd_mmm_yyyy" -v uri_pattern="$uri_pattern" '
    NF > 0 {
        timestamp = $1;
        username = $2;
        request = $3;
...
if [ "$BUNDLE_TYPE" = "jira" ] || [ "$BUNDLE_TYPE" = "both" ]; then
    ...
    if [ -n "$JIRA_BUNDLES" ]; then
        ...
        for bundle in $JIRA_BUNDLES; do
            bundle_data=$(analyze_jira_logs "$bundle" "$userid" "$DATE_DD_MMM_YYYY" "$URI_PATTERN")
...
$bundle_data"
        ...
        if [ -n "$all_jira_data" ]; then
            ...
            unique_jira_data=$(echo "$all_jira_data" | sort -u)
            calculate_statistics "$unique_jira_data" "$userid"
        else
...
if [ "$BUNDLE_TYPE" = "confluence" ] || [ "$BUNDLE_TYPE" = "both" ]; then
    ...
    if [ -n "$CONFLUENCE_BUNDLES" ]; then
        ...
        for bundle in $CONFLUENCE_BUNDLES; do
            bundle_data=$(analyze_confluence_logs "$bundle" "$userid" "$DATE_DD_MMM_YYYY" "$URI_PATTERN")
            if [ -n "$bundle_data" ]; then
...
$bundle_data"
        ...
        if [ -n "$all_confluence_data" ]; then
            # Remove duplicates across all bundles before calculating statistics
            unique_confluence_data=$(echo "$all_confluence_data" | sort -u)
            calculate_statistics "$unique_confluence_data" "$userid"
        else
