...
# 
# Usage: ./analyze_user_simple.sh <USERID> <DATE> [BUNDLE_TYPE]
# 
# Author: SRE Team
# Version: 1.0
# ========================================================================================
...
analyze_confluence() {
    ...
    local output=$(echo "$extracted_data" | awk -F'|' -v userid="$userid" -v target_date="$DATE_DD_MMM_YYYY" '
    NF > 0 {
        timestamp = $1;
        username = $2;
        request = $3;
...
if [ "$BUNDLE_TYPE" = "jira" ] || [ "$BUNDLE_TYPE" = "both" ]; then
    ...
    if [ -n "$JIRA_BUNDLES" ]; then
        ...
        for bundle in $JIRA_BUNDLES; do
            bundle_data=$(analyze_jira "$bundle" "$userid")
            [ -n "$bundle_data" ] && all_data="$all_data
$bundle_data"
        done
        
        calc_stats "$all_data" "$userid" "Jira"
    else
...
if [ "$BUNDLE_TYPE" = "confluence" ] || [ "$BUNDLE_TYPE" = "both" ]; then
    ...
    if [ -n "$CONFLUENCE_BUNDLES" ]; then
        ...
        for bundle in $CONFLUENCE_BUNDLES; do
            bundle_data=$(analyze_confluence "$bundle" "$userid")
        [ -n "$bundle_data" ] && all_data="$all_data
$bundle_data"
    done
        calc_stats "$all_data" "$userid" "Confluence"
    else
